'''
    Code used to train our BNN Q-Learning agent
'''
...
#from model import BayesNetwork
from skimage.color import rgb2gray
from skimage.transform import resize
from torchbnn import BKLLoss, BayesConv2d, BayesLinear
...
class DQNAgent:
    def __init__(self, action_size):
        ...
        self.model = nn.Sequential(
            ...
            nn.ReLU(),
            BayesConv2d(prior_mu=0, prior_sigma=0.1, in_channels = 64, out_channels = 64, kernel_size = 3, stride = 1),
            nn.ReLU(),
            nn.Flatten(),
            nn.Linear(64 * 7 * 7, 512),
            nn.ReLU(),
            BayesLinear(prior_mu=0, prior_sigma=0.1, in_features=512, out_features=self.action_size)
        ).cpu()
        ...
        self.klloss = BKLLoss(reduction="mean", last_layer_only=False)
        ...
        if self.load_model:
            self.model.load_weights("./plot_BNN/models/model.ckpt")
    ...
    def compute_loss(self, history, action, target):
        ...
        a_one_hot = to_one_hot(action, self.action_size).cpu()
        ...
        klloss = self.klloss(self.model)
        loss += 0.0005 * klloss[0]
    ...
    def get_action(self, history):
        history = torch.tensor(history / 255.0, dtype = torch.float).cpu()
    ...
    def train_replay(self):
        ...
        history = torch.cat(history, dim = 0).cpu()
        next_history = torch.cat(next_history, dim = 0).cpu()
        ...
        target = torch.tensor(target, dtype = torch.float).cpu()
    ...
    def save_model(self):
        torch.save(self.model.state_dict(), './plot_BNN/models/bayes_model.ckpt')
    ...
    def log_and_reset(self):
        ...
        np.save('./plot_BNN/logs/bayes_q_log.npy', np.array(self.q_log, dtype = np.float))
        np.save('./plot_BNN/logs/bayes_loss_log.npy', np.array(self.loss_log, dtype = np.float))
        np.save('./plot_BNN/logs/bayes_unclipped_log.npy', np.array(self.unclipped_log, dtype = np.float))
        np.save('./plot_BNN/logs/bayes_clipped_log.npy', np.array(self.clipped_log, dtype = np.float))
...
if __name__ == "__main__":
    ...
    while True:
        ...
        while not done:
            ...
            q_val = agent.model(torch.tensor(history / 255, dtype = torch.float).cpu())
            ...
            if done:
                print("episode:", e, 
                      ...
                      "\nepisode duration:", np.round(time() - t,1), 
                      )
                e += 1
                agent.log_and_reset()
